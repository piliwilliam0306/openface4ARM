...
def train(label):
    ...
    rospy.sleep(6)
    #os.system('rosrun openface4ARM ros_image_crop.py')
...
if __name__ == '__main__':
    ...
    service1 = rospy.Service('whoami', FaceRec, infer)
    service2 = rospy.Service('trainning', Train, train)
...
Use `--networkModel` to set a non-standard Torch network model.""")
    ...
    #    train(args)
        ...
        #infer(args)
        #infer()
        #infer(recDir, args.multi)
        #person, confidence = infer(args, args.multi)
    	#print("Woolala! Predict {} with {:.2f} confidence.".format(person, confidence))	
    rospy.spin() 
